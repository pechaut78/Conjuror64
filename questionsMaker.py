...
prompt = PromptTemplate(
    ...
    answers should be verbose and detailed with valuable information.
    If you redfer to a code snipet, please include its source code in the question and answer.
    only create  questions whose answer is in the text and are about the core knowledge of the text: no easy and no generic questions about generic facts.
...
def handleQA(fname,text):
    ...
    if os.path.exists(csv_filename):
        ...
        csv_filename = f"{fname}_{random_number}.csv"
...
def main():
    ...
    global model
    model = ChatGroq(groq_api_key=api_key,model_name=groq_model, temperature=0.4)
    print(model)
    global chain
